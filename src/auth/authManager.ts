    ...
    while (retryCount < maxRetries) {
      try {
        ...
        if (userSidebar) {
          ...
          if (isLoggedIn) {
            logger.info('Already logged in to main site');
            
            // Also login to creator platform
            logger.info('Checking creator platform login status');
            await this.page.goto('https://creator.xiaohongshu.com', {
              waitUntil: 'domcontentloaded',
              timeout: timeoutMs
            });

            // Check if creator platform needs login
            const creatorLoginNeeded = await this.page.url().includes('/login');
            if (creatorLoginNeeded) {
              logger.info('Creator platform requires login, redirecting user');
              console.log('\n=== 需要登录创作者平台 ===');
              console.log('请在打开的浏览器中完成创作者平台的登录');
              console.log('登录完成后，程序将自动继续...\n');
              
              // Wait for user to complete creator platform login
              await this.page.waitForFunction(() => {
                return !window.location.href.includes('/login');
              }, { timeout: timeoutMs * 6 });
              
              logger.info('Creator platform login completed');
            }

            // Save all cookies (including creator platform)
        ...
        logger.info('Main site login successful');

        // Also login to creator platform
        logger.info('Navigating to creator platform');
        await this.page.goto('https://creator.xiaohongshu.com', {
          waitUntil: 'domcontentloaded',
          timeout: timeoutMs
        });
        ...
        const creatorLoginNeeded = await this.page.url().includes('/login');
        if (creatorLoginNeeded) {
          logger.info('Creator platform requires login, waiting for user');
          console.log('\n=== 需要登录创作者平台 ===');
          console.log('请在打开的浏览器中完成创作者平台的登录');
          console.log('登录完成后，程序将自动继续...\n');
